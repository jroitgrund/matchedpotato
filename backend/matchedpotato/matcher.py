...
from matchedpotato.vinted import SearchRequest, VintedResult, get_vinted_results
...
async def get_matching_vinted_results(
    ...
    results_generator = get_vinted_results(searchRequest)
    try:
        async for new_results in results_generator:
            ...
            scored = [
                (_score(searchRequest.color, result), result) for result in unseen
            ]
            results = sorted(
